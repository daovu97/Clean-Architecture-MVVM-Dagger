...
class RepositoryImpl(
    ...
    override suspend fun getEmployeeById(id: String, result: (Employee?, Throwable?) -> Unit): Job =
        withContext(Dispatchers.IO) {
            launch {
                try {
                    result(Convert.employeeEntityToEmployee(apiService.getEmployee(id)), null)
                } catch (e: Throwable) {
                    result(null, e)
                }
            }
    ...
    override suspend fun createEmployee(
        ...
        launch {
            try {
                apiService.creatEmployee(Convert.employeeToEmployeeEntity(employee))
                result(true, null)
            } catch (e: Throwable) {
                result(false, e)
            }
    ...
    override suspend fun updateEmployee(
        ...
        launch {
            try {
                apiService.updateEmployee(Convert.employeeToEmployeeEntity(employee).id!!)
                result(true, null)
            } catch (e: Throwable) {
                result(false, e)
            }
